...
2. Rename the domain throughout the codebase:
   ...
   - Add `country` to `hacs.json` **only** if your device or service is country-specific, e.g. `"country": ["BR"]` (ISO 3166-1 alpha-2, string or list). HACS hides the repository from users who filter by another country, so leave it out for anything usable worldwide — the blueprint ships without it on purpose. Once set, that country's language becomes the repository language — README, docstrings, comments, commits, PRs, changelog headings, `.github/` templates and public replies — while the code stays in English and native domain terms stay untranslated (see `CODE_STYLE.md`, "Language").
